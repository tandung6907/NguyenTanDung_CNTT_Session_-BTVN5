const STORAGE_KEY = "tet_todos_v6";
...
function clearAll() {
  if (todos.length === 0) {
    showToast("Danh sách đã trống rồi! 🎋");
    return;
  }
  showConfirm(
    ...
    () => {
      ...
      saveTodos(todos);
      render(todos);
      showToast("Đã xóa tất cả công việc! 🗑");
    },
    () => {
      showToast("Đã hủy thao tác xóa tất cả!");
    },
    "Xóa tất cả",
  );
}

// Hàm hiện confirm box tự làm — hỗ trợ tuỳ chỉnh nhãn nút OK
function showConfirm(message, onOk, onCancel, okLabel = "Xóa") {
